...
document.getElementById("BUSCARautor").addEventListener("click", () => {
  ...
  biblioteca.autores.forEach(autor => {
    if (autor.idAutor === id && autor.nombre == nombre && autor.apellido === apellido) {
      ...
      ["ID","Nombre","Apellido", "Libro","Genero"].forEach(texto => {
...
document.getElementById("BUSCARgenero").addEventListener("click", () => {
  ...
  let encontrado = false;
  biblioteca.generos.forEach(genero => {
    if (genero.idGenero === id && genero.nombre === nombre) {
      encontrado = true;
      let div = document.getElementById("miDiv");
      div.innerHTML = "";
      let tabla = document.createElement("table");
      ...
      ["ID", "Genero", "Libro", "Autor"].forEach(texto => {
        let th = document.createElement("th");
        th.textContent = texto;
        filaTitulo.appendChild(th);
      });
      thead.appendChild(filaTitulo);
      biblioteca.libros.forEach(libro => {
        if (libro.codGenero == genero.idGenero) {
          let nombreAutor = "";
          biblioteca.autores.forEach(autor => {
            if (autor.idAutor == libro.codAutor) {
              nombreAutor = autor.nombre + " " + autor.apellido;
            }
          });
          let fila = document.createElement("tr");
          fila.innerHTML = `<td>${genero.idGenero}</td>
              <td>${genero.nombre}</td>
              <td>${libro.titulo}</td>
              <td>${nombreAutor}</td>`;
          tbody.appendChild(fila);
...
document.getElementById("MODIFICARgenero").addEventListener("click", () => {
  let id = document.getElementById("IDgenero").value;
  let nombre = document.getElementById("NOMBREgenero").value;
  let encontrado = false;
  biblioteca.generos.forEach(genero => {
...
document.getElementById("AGREGARlibro").addEventListener("click", () => {
  ...
  let fecha = document.getElementById("FECHAlibro").value;
  let paginas = document.getElementById("NUMlibro").value;
  let codAutor = document.getElementById("IDautorLibro").value;
  let codGenero = document.getElementById("IDgeneroLibro").value;
  let existeID = false;
  ...
  if (!autorExiste) {
    alert("El código de autor no existe");
    return;
  }
  if (!generoExiste) {
    alert("El código de género no existe");
    return;
  }
...
document.getElementById("BUSCARlibro").addEventListener("click", () => {
    ...
    let encontrado = false;
    biblioteca.libros.forEach(libro => {
      if (libro.IDLibro == id && libro.titulo.toLowerCase() === titulo.toLowerCase()
      ) {
        encontrado = true;
        let div = document.getElementById("miDiv");
        div.innerHTML = "";
        ...
        let tbody = document.createElement("tbody");
        let filaTitulo = document.createElement("tr");
        ["ID", "Titulo", "Fecha", "Paginas", "Autor", "Genero"].forEach(texto => {
          let th = document.createElement("th");
          th.textContent = texto;
          filaTitulo.appendChild(th);
        });
        ...
        let fila = document.createElement("tr");
        fila.innerHTML = `<td>${libro.IDLibro}</td>
          <td>${libro.titulo}</td>
...
document.getElementById("MODIFICARlibro").addEventListener("click", () => {
    let id =document.getElementById("IDlibro").value;
    let titulo =document.getElementById("TITULOlibro").value;
    let fecha =document.getElementById("FECHAlibro").value;
    let paginas = document.getElementById("NUMlibro").value;
...
`;
  ...
  biblioteca.libros.forEach(libro => {
      let autor =biblioteca.autores.find(a =>a.idAutor==libro.codAutor);

      let genero = biblioteca.generos.find(g => g.idGenero == libro.codGenero);
      let fila = document.createElement("tr");
      fila.innerHTML =
        `
<td>${libro.IDLibro}</td>
